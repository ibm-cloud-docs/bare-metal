...
Embora a maioria dos clientes {{site.data.keyword.cloud}} use um dos Sistemas Operacionais padrão que vêm com os nossos servidores, é possível montar os ISOs customizados (imagens do disco) em servidores. Você tem três opções para montar ISOs customizados.

Para que os métodos funcionem, é necessário se conectar à rede privada por meio do serviço VPN SL, como
[SoftLayer SSL
VPN Portal - AMS01](https://vpn.ams01.softlayer.com/prx/000/http/localhost/login) ou por meio de outro servidor conectado à rede.
...
Se você já tiver a infraestrutura implementada no {{site.data.keyword.cloud_notm}}, será possível configurar um servidor existente para oferecer um compartilhamento CIFS para a rede interna. Será possível então montar qualquer ISO em um servidor bare metal.
...
* Efetue login no console de gerenciamento do IPMI apontando seu navegador da web
para o IP especificado em cloud.ibm.com e em dispositivos -> seu servidor (detalhes do
dispositivo) -> Gerenciamento remoto. O nome do usuário e a senha também são especificados aqui.
...
* Estabeleça uma Conexão VPN para [VPN](http://www.softlayer.com/VPN-Access). Se
você estiver usando o Microsoft Internet Explorer, certifique-se de incluir
.softlayer.com e .cloud.ibm.com em sua lista de Sites confiáveis e de manter seu JAVA
atualizado.
...
* Conecte-se ao Servidor sobre o IP de Gerenciamento.<br>
      1. Conecte-se ao `winadmin`.
      2. Abra o IPMIView e acesse **Arquivo > Novo >Sistema**.
      3. Use o endereço IP do IPMI a partir do objeto de hardware para preencher os
campos Nome do servidor e Endereço IP.<br>
      4. Dê um clique duplo no sistema com o mesmo endereço IP no lado esquerdo e
insira ADMIN para o ID de login e a senha do IPMI a partir do objeto de hardware.
      5. Depois de se conectar, você tem muitas guias disponíveis na janela. É
possível usar o **Console de texto** ou o **Console do
KVM** para se conectar ao servidor.

* Abra a guia de Mídia virtual
* Conclua os detalhes da conexão da imagem de CD-ROM.
  *
    * Host de compartilhamento = o Endereço IP do Armazenamento NAS. É possível localizar esse valor efetuando ping do nome do servidor de armazenamento NAS. Por exemplo,  ` ping nas501.service.softlayer.com `
...
1. Efetue login no console de gerenciamento do IPMI apontando seu navegador da web
para o IP que está especificado em cloud.ibm.com.
...
* Ao montar um ISO, use [VPN SSL](http://vpn.softlayer.com) em vez de VPN PPTP. Depois
de conectado à rede VPN, também é possível acessar o IPMI do sistema por meio do
endereço IPMI (https://private-ip-IPMI-management).
* Ao inserir um caminho para um ISO, use a Sintaxe de nome UNC (Convenção
Universal de Nomenclatura) para o caminho, por exemplo:
