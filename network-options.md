...
You can request additional IP addresses for your server (which is recommended when announcing services externally) during server ordering. The **Public secondary IP addresses** option is in the **Add-ons** section of the network options. Select the number of IP addresses you want, then a [secondary static subnet](/docs/infrastructure/subnets?topic=subnets-about-subnets-and-ips#static-subnets) of the appropriate size will automatically be provisioned and routed to your server's public primary IP address. If you require more IP addresses later, you can [order additional secondary subnets](/docs/infrastructure/subnets?topic=subnets-getting-started#ordering-subnets).

## Server hardware firewall
{: #network-firewall-server}

Applicable when a public network interface is requested, this option places a firewall in front of your server. The rules of the firewall apply only to the IP addresses associated with the server. Read [Hardware Firewalls (Shared)](/docs/infrastructure/hardware-firewall-shared?topic=hardware-firewall-shared-about-hardware-firewall-shared-) to learn more.
...
- **Primary IP addresses** - A private IPv4 address is included with a private network interface. If a public network interface is selected, a public IPv4 address is also included. These addresses provide basic connectivity to the server. Learn more about [primary IP addresses](/docs/infrastructure/subnets?topic=subnets-about-subnets-and-ips#primary-subnets).
- **VPN Management** - Manage access to the subnet the server resides on for users when connecting through [VPN](/docs/infrastructure/iaas-vpn?topic=VPN-about-iaas-vpn).
