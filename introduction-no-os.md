...
Install an operating system on to a bare metal server with no operating system by booting from an ISO via the included IPMI device. Click [here](/docs/bare-metal/mount-iso-bare-metal-server.html) for additional information on booting from an ISO.
